...
export function apply(ctx, config = {}) {
  ...
  ctx.on('system-prompt/assemble', async (assembly, context, next) => {
    ...
    try {
      settled = await next()
      if (!Array.isArray(settled?.tools) || settled.tools.length === 0) return settled
      const mode = resolveMode(context)
      if (mode === undefined) {
        if (policyOf() === undefined) noteMissingPolicy()
        return settled
      }
      const tools = narrowTools(settled.tools, mode)
      return tools === settled.tools ? settled : { ...settled, tools }
    } catch (error) {
