...
export function apply(ctx, config = {}) {
  ...
  // A host that mounts an enforcing executor must also mount the policy the
  // mode is resolved from; without it this plugin would narrow schemas against
  // the composition default instead of the session's real mode.
  if (policy === undefined && config.resolveMode === undefined && warn) {
    console.warn('[sandbox-escalation-guard] ctx.sandboxPolicy is not composed: escalation schemas cannot be narrowed per session. Mount @deepseek-ai/dsh-sandbox-policy alongside this plugin.')
  }

  ctx.on('system-prompt/assemble', (assembly, context) => {
    try {
      if (!Array.isArray(assembly?.tools) || assembly.tools.length === 0) return assembly
      const tools = narrowTools(assembly.tools, resolveMode(context))
      return tools === assembly.tools ? assembly : { ...assembly, tools }
    } catch (error) {
      // Never break a model request over an advisory schema rewrite.
  ...
  ctx.on('tools/pre-execute', (exec, next) => {
    try {
      ...
      if (effective === undefined) return next()
